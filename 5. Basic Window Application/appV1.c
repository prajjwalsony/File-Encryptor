    ...
    switch (msg) {
        ...
        case WM_COMMAND:
            if (LOWORD(wParam) == ID_FILE_BTN) {
                ...
                double expected = (double)size.QuadPart / (1024.0 * 1024.0) / 23.0; // 23 MB/s
                char estmsg[100];
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    wc.lpszClassName = "LiteCryptWin";
    RegisterClass(&wc);

    HWND hwnd = CreateWindow("LiteCryptWin", "LiteCrypt File Encryptor", WS_OVERLAPPEDWINDOW,

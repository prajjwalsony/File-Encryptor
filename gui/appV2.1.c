...
void ShowError(HWND hwnd, int code) {
    const char* messages[] = {
        "Key not match during decryption", "Invalid file name or cannot open file", "Password format incorrect (Exactly 4 characters)",
    ...
    switch (msg) {
        ...
        case WM_COMMAND:
            if (LOWORD(wParam) == ID_FILE_BTN) {
                ...
                MessageBox(hwnd, "Don't Click Anywhere During Processing\nClick OK to start", "Warning", MB_ICONEXCLAMATION );
                ...
                if (result > 0) {
                    ...
                    MessageBox(hwnd, "Successfully Completed", "Success", MB_OK | MB_ICONINFORMATION);
                } else {
